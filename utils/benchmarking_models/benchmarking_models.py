...
class LinearLayer(nn.Module):
    """A linear layer. It is to be used after the above defined models, which in this implementation are not directly
    followed by a final classification layer. Often this final linear layer is trained independently. I will also use it as
    an embedding layer in the DICE algorithm. I will finally use it to compute the variance in reparametrization trick
    component of DICE. For the final use I will need to place a softplus after the hidden representations."""
    num_outputs: int
...
class VariancePredictor(nn.Module):
    """
    A predictor of variances used in the DICE model.
    """
    num_outputs: int

    def setup(self):
        self.block = LinearLayer(num_outputs=self.num_outputs)
...
DiceDescriminator = partial(Descriminator, hidden1=256, hidden2=256, hidden3=100)
